...
export default function AdminClient() {
  ...
  if (!token) {
    return <main className="loginShell"><section className="loginCard">
      <div className="brand loginBrand"><div className="brandMark">↔</div><div className="brandText"><strong>OpenRide</strong><span>Operator</span></div></div>
      <h1>Đăng nhập vận hành</h1><p className="muted">OTP chỉ được gửi tới số điện thoại Operator/Admin đã bootstrap trên backend.</p>
  ...
  return <div className="shell">
    <aside className="sidebar">
      <div className="brand"><div className="brandMark">↔</div><div className="brandText"><strong>OpenRide</strong><span>Operator</span></div></div>
      <nav className="nav" aria-label="Điều hướng quản trị">
        {['▦ Tổng quan','⇄ Công việc','◉ Tài xế','◎ Khách hàng','▣ Phương tiện','₫ Giá & guardrails','! Sự cố','⌕ Nhật ký','⚙ Cài đặt'].map((item,i)=><div className={`navItem${i===0?' active':''}`} key={item}>{item}</div>)}
      </nav>
      <div className="sidebarFooter"><strong style={{color:'white'}}>OpenRide Operations</strong><br/>Runtime compatibility API đang được nâng cấp lên Marketplace V2.</div>
    </aside>
    <main className="main">
      <header className="topbar"><h1>Trung tâm vận hành</h1><div className="topbarRight"><button className="linkButton" onClick={()=>void load()} disabled={busy}>Làm mới</button><span className="env">LIVE</span><button className="avatar" onClick={()=>void logout()}>OP</button></div></header>
      <div className="content" aria-busy={busy}>
        {busy && <div className="loadingBar" aria-label="Đang đồng bộ dữ liệu"><span /></div>}
        <section className="pageHeading"><div><div className="eyebrow">OPENRIDE OPERATOR</div><h2>Trung tâm điều hành mobility</h2><p>Giám sát tài xế, công việc, an toàn và runtime hiện tại trong quá trình chuyển sang Marketplace V2.</p></div><div className="liveBadge"><span className="liveDot"/> Backend connected</div></section>
