...
export default function AdminClient() {
  ...
  const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [activeView, setActiveView] = useState<ViewKey>('overview'); const [mobileNavOpen, setMobileNavOpen] = useState(false);
  ...
  const load = useCallback(async (access=token) => { if (!access) return; setBusy(true); try { const [dashboard,driverData,tripData] = await Promise.all([authedApi<Metrics>('/v1/admin/dashboard',{},access), authedApi<Driver[]>('/v1/admin/drivers',{},access), authedApi<Trip[]>('/v1/admin/trips',{},access)]); setMetrics(dashboard); setDrivers(driverData); setTrips(tripData); setError(''); } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Không thể tải dữ liệu'); } finally { setBusy(false); } }, [authedApi, token]);
  ...
  function renderView() {
    ...
    if (activeView === 'incidents') return <Section title='Sự cố' description='Các công việc cần Operations xử lý.'><DataTable columns={tripColumns} data={incidents} searchPlaceholder='Tìm sự cố...' /></Section>;
    ...
    return <div className='flex flex-col gap-5 fx-enter'>
      <section className='fx-flowline overflow-hidden rounded-[30px] border border-primary/10 bg-gradient-to-br from-white via-white to-primary-soft p-5 shadow-sm md:p-7'>
      ...
      <div className='grid gap-4 sm:grid-cols-2 xl:grid-cols-4'>{metricsCards.map(({label,value,foot,icon:Icon}) => <Card key={label} className='group transition duration-200 hover:-translate-y-0.5 hover:shadow-md'><CardContent className='p-5 md:p-5'><div className='flex items-start justify-between gap-4'><div><p className='text-sm font-medium text-muted'>{label}</p><p className='mt-2 text-3xl font-bold tracking-[-.04em] text-foreground'>{value}</p><p className='mt-1 text-xs text-muted'>{foot}</p></div><div className='grid size-11 place-items-center rounded-2xl bg-primary-soft text-primary'><Icon aria-hidden='true' className='size-5' /></div></div></CardContent></Card>)}</div>

      <div className='grid gap-5 xl:grid-cols-[1.4fr_.6fr]'>
        <Card><CardHeader className='flex-row items-start justify-between'><div><CardTitle>Công việc gần đây</CardTitle><CardDescription>Dữ liệu trực tiếp từ backend.</CardDescription></div><Button size='sm' variant='ghost' onClick={() => selectView('trips')}>Xem tất cả <ChevronRight aria-hidden='true' className='size-4' /></Button></CardHeader><CardContent><DataTable columns={tripColumns} data={trips.slice(0,8)} /></CardContent></Card>
        <Card className='overflow-hidden'><CardHeader><div className='flex items-center justify-between'><div><CardTitle>Chờ duyệt tài xế</CardTitle><CardDescription>Ưu tiên xử lý supply.</CardDescription></div><Badge variant={pendingDrivers.length ? 'warning':'success'}>{pendingDrivers.length}</Badge></div></CardHeader><CardContent className='flex flex-col gap-3'>{pendingDrivers.length ? pendingDrivers.slice(0,5).map(driver => <div key={driver.id} className='rounded-2xl border border-border bg-surface-soft/60 p-4'><div className='flex items-start justify-between gap-3'><div><div className='font-semibold text-foreground'>{driver.full_name || driver.phone || driver.id}</div><div className='mt-1 text-xs text-muted'>{serviceLabel(driver.service_type)}</div></div><Badge variant='warning'>Chờ duyệt</Badge></div><div className='mt-4 flex gap-2'><Button size='sm' variant='destructive' onClick={() => void approve(driver,'rejected')}>Từ chối</Button><Button size='sm' onClick={() => void approve(driver,'approved')}>Duyệt</Button></div></div>) : <InfoState icon={ShieldCheck} title='Đã xử lý hết' text='Không có tài xế nào đang chờ duyệt.' />}</CardContent></Card>
      </div>
    </div>;
  }

  return <div className='min-h-screen lg:grid lg:grid-cols-[264px_1fr]'>
    <aside className='hidden border-r border-border bg-white/80 p-4 backdrop-blur-xl lg:flex lg:min-h-screen lg:flex-col'><Brand /><nav className='mt-7 flex flex-col gap-1'>{navItems.map(({key,label,icon:Icon}) => <button key={key} onClick={() => selectView(key)} className={`flex min-h-11 items-center gap-3 rounded-2xl px-3.5 text-left text-sm font-medium transition ${activeView===key?'bg-primary-soft text-primary':'text-muted hover:bg-surface-soft hover:text-foreground'}`}><Icon aria-hidden='true' className='size-[18px]' /><span>{label}</span>{activeView===key ? <span className='ml-auto size-1.5 rounded-full bg-primary' /> : null}</button>)}</nav><div className='mt-auto rounded-2xl border border-primary/10 bg-primary-soft/55 p-4'><div className='flex items-center gap-2 text-sm font-semibold text-primary'><ShieldCheck aria-hidden='true' className='size-4' /> FlashX Operations</div><p className='mt-2 text-xs leading-5 text-muted'>Ba dịch vụ MVP đang dùng chung một trung tâm vận hành.</p></div></aside>

    {mobileNavOpen ? <div className='fixed inset-0 z-50 bg-foreground/20 p-3 backdrop-blur-sm lg:hidden' onClick={() => setMobileNavOpen(false)}><div className='fx-glass ml-auto flex h-full w-[min(88vw,340px)] flex-col rounded-[28px] p-4' onClick={event => event.stopPropagation()}><div className='flex items-center justify-between'><Brand /><Button size='icon' variant='ghost' onClick={() => setMobileNavOpen(false)} aria-label='Đóng menu'><X aria-hidden='true' className='size-5' /></Button></div><nav className='mt-6 flex flex-col gap-1'>{navItems.map(({key,label,icon:Icon}) => <button key={key} onClick={() => selectView(key)} className={`flex min-h-12 items-center gap-3 rounded-2xl px-4 text-left text-sm font-semibold ${activeView===key?'bg-primary text-white':'text-foreground hover:bg-surface-soft'}`}><Icon aria-hidden='true' className='size-5' />{label}</button>)}</nav></div></div> : null}

    <main className='min-w-0'><header className='sticky top-0 z-30 border-b border-border/80 bg-white/80 backdrop-blur-xl'><div className='flex h-16 items-center justify-between px-4 md:px-6'><div className='flex items-center gap-3'><Button size='icon' variant='ghost' className='lg:hidden' onClick={() => setMobileNavOpen(true)} aria-label='Mở menu'><Menu aria-hidden='true' className='size-5' /></Button><div><div className='text-[11px] font-semibold uppercase tracking-[.13em] text-primary'>FlashX Operations</div><h1 className='text-lg font-semibold tracking-tight text-foreground'>{activeLabel}</h1></div></div><div className='flex items-center gap-2'><Badge variant='success'><span className='mr-1.5 size-1.5 rounded-full bg-success' />LIVE</Badge><Button size='icon' variant='ghost' onClick={() => void load()} disabled={busy} aria-label='Làm mới'><RefreshCw aria-hidden='true' className={`size-4 ${busy?'animate-spin':''}`} /></Button><Button size='icon' variant='ghost' onClick={() => void logout()} aria-label='Đăng xuất'><LogOut aria-hidden='true' className='size-4' /></Button></div></div></header><div className='mx-auto max-w-[1500px] p-4 md:p-6 lg:p-7'>{error ? <div className='mb-4 rounded-2xl border border-danger/15 bg-danger-soft p-4 text-sm text-danger'>{error}</div> : null}{renderView()}</div></main>
  </div>;
}

function Brand() { return <div className='flex items-center gap-3 px-2'><div className='grid size-10 place-items-center rounded-2xl bg-primary text-white shadow-sm'><Zap aria-hidden='true' className='size-5 fill-current' /></div><div><div className='text-base font-bold tracking-tight text-foreground'>FlashX</div><div className='text-[11px] font-medium text-muted'>Operations</div></div></div>; }

function LoginScreen({ challenge, debugCode, busy, error, phoneForm, otpForm, requestOtp, verifyOtp, resetChallenge }: any) {
  return <main className='relative grid min-h-screen place-items-center overflow-hidden bg-background p-5'><div className='absolute left-[-10%] top-[-20%] size-[38rem] rounded-full bg-primary-tint/50 blur-3xl' /><div className='absolute bottom-[-20%] right-[-12%] size-[34rem] rounded-full bg-primary-soft blur-3xl' /><Card className='fx-glass fx-enter relative z-10 w-full max-w-[430px] rounded-[30px]'><CardHeader className='pb-4'><Brand /><div className='pt-5'><CardTitle className='text-2xl font-bold tracking-[-.035em]'>Đăng nhập quản trị</CardTitle><CardDescription>Trung tâm vận hành FlashX · Xác thực bằng OTP.</CardDescription></div></CardHeader><CardContent className='flex flex-col gap-4'>{error ? <div className='rounded-2xl bg-danger-soft p-3.5 text-sm text-danger'>{error}</div> : null}{!challenge ? <form onSubmit={requestOtp} className='flex flex-col gap-4'><label className='flex flex-col gap-1.5 text-sm font-semibold text-foreground'>Số điện thoại<Input {...phoneForm.register('phone')} disabled={busy} placeholder='0999999999' autoComplete='tel' /><span className='min-h-4 text-xs font-normal text-danger'>{phoneForm.formState.errors.phone?.message || ''}</span></label><Button size='lg' className='w-full' type='submit' disabled={busy}>{busy?'Đang xử lý…':'Tiếp tục'}</Button></form> : <form onSubmit={verifyOtp} className='flex flex-col gap-4'><label className='flex flex-col gap-1.5 text-sm font-semibold text-foreground'>Mã OTP<Input {...otpForm.register('code')} inputMode='numeric' autoComplete='one-time-code' maxLength={6} placeholder='000000' /><span className='min-h-4 text-xs font-normal text-danger'>{otpForm.formState.errors.code?.message || ''}</span></label>{debugCode ? <div className='rounded-2xl border border-primary/10 bg-primary-soft p-3.5 text-sm text-primary'>DEV OTP: <strong className='font-mono tracking-wider'>{debugCode}</strong></div> : null}<div className='flex gap-2'><Button type='button' variant='outline' onClick={resetChallenge}>Quay lại</Button><Button className='flex-1' type='submit' disabled={busy}>Xác nhận OTP</Button></div></form>}<p className='text-center text-xs leading-5 text-muted'>Phiên đăng nhập được bảo vệ bằng access token ngắn hạn và refresh token.</p></CardContent></Card></main>;
}
