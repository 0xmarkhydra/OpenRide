...
func (s *Server) publishActor(actorID, eventType, tripID string, data any) {
	if s.deps.Realtime == nil || actorID == "" { return }
...
func (s *Server) publishTrip(trip trips.Trip, eventType string, data any) {
	...
	if trip.DriverID!=""{s.publishActor(trip.DriverID,eventType,trip.ID,data)}
}
...
func (s *Server) publishOffers(offers []dispatch.Offer) {
	for _,offer:=range offers {
		...
		if err!=nil{continue}
		s.publishActor(offer.DriverID,"dispatch.offer",offer.TripID,map[string]any{"offer":offer,"trip":trip})
	}
}
