...
type AcceptQuoteCommand struct {
	QuoteID, RiderID, IdempotencyKey, AgreementID, EventID string
	Now                                                       time.Time
}

type OutboxEvent struct {
	ID, Name, AggregateType, AggregateID, InstanceID string
...
func (s AcceptanceService) AcceptQuote(ctx context.Context, cmd AcceptQuoteCommand) (marketplace.Agreement, error) {
	...
	err := s.Store.WithinTx(ctx, func(tx AcceptanceTx) error {
		if err := tx.LockIdempotency(ctx, cmd.RiderID, cmd.IdempotencyKey); err != nil {
			return err
		}
		...
		if err := tx.AppendOutbox(ctx, OutboxEvent{
			ID:            cmd.EventID,
			Name:          "openride.marketplace.agreement.created.v1",
