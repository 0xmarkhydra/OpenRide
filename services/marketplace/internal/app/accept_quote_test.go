...
func (s *memoryAcceptanceStore) AppendOutbox(_ context.Context, e OutboxEvent) error {
...
func TestAcceptQuoteCommitsAgreementAndOutboxOnce(t *testing.T) {
	...
	store := &memoryAcceptanceStore{
		request: marketplace.Request{ID: "req_1", InstanceID: "default", RiderID: "rider_1", ServiceType: "passenger.car", Status: marketplace.RequestOpen, Pickup: geo.Point{Lat: 21.0278, Lng: 105.8342}, Destination: &destination, RequestedAt: now.Add(-time.Minute), Version: 1},
		quote: marketplace.Quote{ID: "quote_1", RequestID: "req_1", DriverID: "driver_1", TariffID: "tariff_1", TariffVersion: 2, Status: marketplace.QuotePending, Fare: money.Must("VND", 52_000), PickupDistanceM: 900, PickupETAS: 180, Metadata: map[string]any{"source": "driver_tariff"}, CreatedAt: now.Add(-30 * time.Second), ExpiresAt: now.Add(time.Minute)},
		idempotency: map[string]string{},
	}
	service := AcceptanceService{Store: store}
	cmd := AcceptQuoteCommand{QuoteID: "quote_1", RiderID: "rider_1", IdempotencyKey: "idem_1", AgreementID: "agreement_1", EventID: "event_1", Now: now}
